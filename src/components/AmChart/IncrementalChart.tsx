...
const IncrementalChart: React.FC<IncrementalChartProps> = () => {
  ...
  useLayoutEffect(() => {
    ...
    switch (selectedSeriesType) {
      ...
      case "ohlc":
        valueSeries = chart.series.push(
          am5xy.OHLCSeries.new(root, candleStickSettings)
        );
        break;
    }
    ...
    const loadData = async (
      ...
      try {
        ...
        if (side === "none") {
          if (data.length > 0) {
            if (
              dateAxis.get("baseInterval")?.timeUnit !==
              currentInterval.timeUnit
            ) {
    ...
    const loadSomeData = () => {
      const start = dateAxis.get("start") as number;
      const end = dateAxis.get("end") as number;
      const selectionMin = Math.max(
        dateAxis.getPrivate("selectionMin") as number,
        absoluteMin
      );
      const selectionMax = Math.min(
        dateAxis.getPrivate("selectionMax") as number,
        absoluteMax
      );
    ...
    const min =
      currentDate.getTime() -
      am5.time.getDuration(
        currentInterval.timeUnit,
        currentInterval.count * 36
      );
      ...
      items: [
        {
          id: "5 sec",
          label: "5 sec",
          interval: { timeUnit: "second", count: 5 },
        },
        {
          id: "10 sec",
          label: "10 sec",
          interval: { timeUnit: "second", count: 10 },
        },
        {
          id: "15 sec",
          label: "15 sec",
          interval: { timeUnit: "second", count: 15 },
        },
        {
          id: "30 sec",
          label: "30 sec",
          interval: { timeUnit: "second", count: 30 },
        },
        {
          id: "1 Min",
          label: "1 Min",
        ...
        },
        {
          id: "3 Hour",
          label: "3 Hour",
          interval: { timeUnit: "hour", count: 3 },
        },
        {
          id: "1 Day",
          label: "1 Day",
          interval: { timeUnit: "hour", count: 24 },
        },
      ],
    });
    ...
    stockChart.events.on("drawingadded", (ev) => {
      ...
      ev.series.events.on("rightclick", (ev) => {
        ...
        setMenuVisible(true);
        setMenuPosition({
          x: ev.originalEvent.clientX + 10,
          y: ev.originalEvent.clientY + 10,
        });
        ...
        stockChart.events.on("click", (ev) => {
          ...
          const drawControlSettings = document.getElementById(
            "drawControlSettings"
          );
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        drawingToolsElement &&
        !drawingToolsElement.contains(event.target as Node)
      ) {
  ...
  useEffect(() => {
    ...
    const filteredTrades = trades.filter(
      (trade) => trade.symbol === currentSymbol?.name
    );
    ...
    filteredTrades.forEach((trade) => {
      ...
      const closingTime = trade.closingTime
  ...
  const activateDrawingTool = (toolType: string) => {
    ...
    if (toolType === "Eraser") {
      ...
      const lineColorInput = document.getElementById(
        "lineColor"
      ) as HTMLInputElement;
          ...
          <div className="flex md:w-full items-center justify-start pl-5 pt-[1.5vh] h-[3vh]">
            ...
            <div id="drawing-tools-toggle" className="relative">
              ...
              {isDropdownOpen && (
                <div
                  id="drawing-tools"
                  className="absolute left-0 bg-gray-900 shadow-lg z-10"
                >
